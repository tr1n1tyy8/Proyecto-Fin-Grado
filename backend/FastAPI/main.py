...
def startup_event():
    try:
        ...
        print(f"⚠️ No se pudieron sincronizar las tablas al arrancar: {e}")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], # Permitimos temporalmente todos para evitar fallos en producción
...
@app.get("/")
async def index():
    """Servir la página de inicio o redirigir a docs si no existe"""
